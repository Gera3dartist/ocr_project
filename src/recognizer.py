...
def load_templates(path: str) -> dict[int, np.ndarray]:
    ...
    Returns:
        Dict mapping digit value (0-9) to template image.
    """
    data = np.load(path)
...
def recognize_digit(
    digit_img: np.ndarray, templates: dict[int, np.ndarray]
) -> tuple[int, float]:
    ...
    for digit, tmpl in templates.items():
        ...
        # Pixel overlap (IoU of white pixels)
        img_white = img > 127
        tmpl_white = tmpl > 127
        ...
        combined = 0.6 * correlation + 0.4 * overlap
        scores.append((digit, combined))
...
def detect_transition(
    ...
    for digit, tmpl in templates.items():
        ...
        top_scores.append((digit, top_iou))

        # Compare bottom halves
        ...
        bottom_scores.append((digit, bot_iou))
...
def recognize_all(
    digit_images: list[np.ndarray], templates: dict[int, np.ndarray]
) -> MeterReading:
